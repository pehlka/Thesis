...
lme.log.side.vmax.subtukey  #some significant differences due to gait
lme.log.side.vmax.subtukey2 <- emmeans(lme.log.side.vmax.sub, pairwise ~ substrate_type, adjust="tukey", data=lab_data2)
lme.log.side.vmax.sub.tukey2   #D-S p=0.0001, D-SD p=0.0074, S-SD p=0.3280
...
lme.log.side.amax.tukey <- emmeans(lme.log.side.amax.sub, pairwise ~ substrate_type, adjust="tukey", data=lab_data2)
lme.log.side.amax.tukey  #no significant differences in any substrate's acceleration
...
```{r}
#Speed ~ Substrate
mcelroy_speed_sub_lm <- lm(log10(Speed) ~ Substrate.Type, data = fieldspeed_jan, na.action=na.omit)
anova(mcelroy_speed_sub_lm)   #p=0.2723
...
mcelroy_speed_clutter_lm <- lm(log(Speed+1) ~ log(NumberClutter+1), data = fieldspeed_reduced_debris, na.action = na.omit)
