...
lme.log.side.amax.sub <- lme(log10(side_amax+1) ~ substrate_type + sex, random = ~1|ID, data = lab_data)
...
lme.biped.sub <- lme(sqrt(bipedal_percent) ~ substrate_type + sex, random = ~1|ID, data = lab_data_omit)
...
mcelroy_speed_sub_lme2 <- lme(log10(Speed) ~ Substrate.Type + Sex, random = ~1|ID, data = fieldspeed_reduced, na.action=na.omit)
...
side.amax.biped <- lme(log10(side_amax+1) ~ log10(bipedal_percent+1) + sex, random = ~1|ID, data = lab_data_omit)
...
```{r}
table(lab_data$bipedal_quad_side)  #total = 139
#sum(lab_data$bipedal_side)
#sum bipedal_side = 5390 frames bipedal
table(lab_data$bipedal_quad_top)  #total = 139
