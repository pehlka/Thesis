...
> Testing effect of sex, SVL, and tail length on velocity, acceleration and bipedalism
...
ggplot(lab_data2, aes(residuals(lme.log.side.vmax.sex))) + 
  geom_histogram(bins = 11, color = "white") +
  labs(x = "Residuals of lme.log.side.vmax.sex") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), 
        axis.line = element_line(color = "black"))
...
lme.log.side.vmax.sex.tukey <- emmeans(lme.log.side.vmax.sex, pairwise ~ sex, adjust="tukey", data=lab_data2)
lme.log.side.vmax.sex.tukey 

#sex is insignificant for peak velocity
```

```{r}
#Acceleration vs Sex - Side
lme.log.side.amax.sex <- lme(log10(side_amax+1) ~ sex, random = ~1|ID, data = lab_data2)
anova(lme.log.side.amax.sex)  #p-value = 0.7016, not significant
ggplot(lab_data2, aes(residuals(lme.log.side.amax.sex))) + 
  geom_histogram(bins = 8, color = "white") +
  labs(x = "Residuals of lme.log.side.amax.sex") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), 
        axis.line = element_line(color = "black"))
shapiro.test(residuals(lme.log.side.amax.sex))  #normal
summary(lme.log.side.amax.sex)
performance::r2(lme.log.side.amax.sex)   #C=0.054  #M=0.001
lme.log.side.amax.sex.tukey <- emmeans(lme.log.side.amax.sex, pairwise ~ sex, adjust="tukey", data=lab_data2)
lme.log.side.amax.sex.tukey

#sex is insignificant for acceleration
```

```{r}
#Bipedalism vs Sex
lme.biped.sex <- lme(sqrt(bipedal_percent) ~ sex, random = ~1|ID, data = lab_data2_omit)
anova(lme.biped.sex)  #p-value = 0.2085, insignificant
ggplot(lab_data2_omit, aes(residuals(lme.biped.sex))) + 
  geom_histogram(bins = 7, color = "white") +
  labs(x = "Residuals of lme.biped.sex") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), 
        axis.line = element_line(color = "black"))
shapiro.test(residuals(lme.biped.sex))   #normal
summary(lme.biped.sex)
performance::r2(lme.biped.sex)  #C:0.387, M: 0.033  
lme.biped.sex.tukey <- emmeans(lme.biped.sex, pairwise ~ sex, adjust="tukey", data=lab_data2_omit)
lme.biped.sex.tukey 

#sex is insignificant on bipedalism
```
...
```{r}
#ggplot for velocity  vs. substrate separated into bipedalism for combined
