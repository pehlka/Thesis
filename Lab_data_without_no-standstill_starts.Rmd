...
fieldspeed_final2 <- read.csv("~/College of Charleston/Thesis/Data/McElroy Habitat Data/fieldspeed_final2.csv")  
  #relabeled IDs to letters instead of numbers
...
```{r}
is.factor(fieldspeed_final$ID)  #numeric
is.factor(fieldspeed_final3$ID)
...
mcelroy_speed_sub_lmer <- lmer(Speed ~ Substrate.Type + (1|ID), data = fieldspeed_final)
anova(mcelroy_speed_sub_lmer)
mcelroy_speed_sub_lmer_tukey <- emmeans(mcelroy_speed_sub_lmer, pairwise ~ Substrate.Type, adjust = "tukey", data=fieldspeed_final)
mcelroy_speed_sub_lmer_tukey  #no significant differences between any of the substrate types
ggplot(data = fieldspeed_final, aes(x = Substrate.Type, y = Speed)) +
  geom_boxplot() +
  scale_x_discrete(labels = c("?", "Debris", "Mixed", "Sand")) +
  labs(x = "Substrate Type", y = bquote('Speed' ~ (m^2/s))) +
  theme(panel.background = element_blank()) + 
  theme(axis.line.x = element_line(color = "black", size = 0.5),
        axis.line.y = element_line(color = "black", size = 0.5))

shapiro.test(residuals(mcelroy_speed_sub_lmer))  #normal
hist(residuals(mcelroy_speed_sub_lmer))
performance::r2(mcelroy_speed_sub_lmer)  #R2 kinda low #C=0.175  #M=0.040
```

```{r}
#Lmer using fieldspeed_final2, where IDs were changed to letters instead of numbers (to see if could help with lme since it's not working) and added IDs to last 4 runs 

#Speed vs. Substrate Type 2
mcelroy_speed_sub_lmer2 <- lmer(Speed ~ Substrate.Type + (1|ID), data = fieldspeed_final2)
anova(mcelroy_speed_sub_lmer2)
mcelroy_fieldspeed_lmer_tukey2 <- emmeans(mcelroy_speed_sub_lmer2, pairwise ~ Substrate.Type, adjust = "tukey", data=fieldspeed_final2)
mcelroy_fieldspeed_lmer_tukey2  #no significant differences between any of the substrate types
ggplot(data = fieldspeed_final2, aes(x = Substrate.Type, y = Speed)) +
  geom_boxplot() +
  scale_x_discrete(labels = c("?", "Debris", "Mixed", "Sand")) +
  labs(x = "Substrate Type", y = bquote('Speed' ~ (m^2/s))) +
  theme(panel.background = element_blank()) + 
...
shapiro.test(residuals(mcelroy_speed_sub_lmer2))  #normal
hist(residuals(mcelroy_speed_sub_lmer2))
performance::r2(mcelroy_speed_sub_lmer2)  #R2 kinda low #C=0.151  #M=0.029

#R2 slightly different from fieldspeed_final analysis, but besides that everything else is pretty much the same
```

```{r}
#Speed vs. Substrate Type - lme - fieldspeed_final3 dataset
fieldspeed_final3 <- read.csv("~/College of Charleston/Thesis/Data/McElroy Habitat Data/fieldspeed_final3.csv")

mcelroy_speed_sub_lme <- lme(Speed ~ Substrate.Type, random = ~1|ID, data = fieldspeed_final3)
anova(mcelroy_speed_sub_lme)  #p-value=0.7666  #not significant
mcelroy_speed_sub_lme_tukey <- emmeans(mcelroy_speed_sub_lme, pairwise ~ Substrate.Type, adjust = "tukey", data = fieldspeed_final3)
...
#lme wouldn't work for some reason using data sets 1 & 2 but when I deleted rows that didn't have a speed value, it worked

#Substrate type doesn't appear to affect speed at all

anova(lme(Speed ~ Substrate.Type + Sex, random = ~1|ID, data = fieldspeed_final3))
...
table(fieldspeed_final$Substrate.Type == "DEBRIS")  #3  (6.5%)
table(fieldspeed_final$Substrate.Type == "MIXED")   #11 (23.9%)
...
#Number Clutter vs Speed

```{r}
fieldspeed_final4 <- read.csv("~/College of Charleston/Thesis/Data/McElroy Habitat Data/fieldspeed_final4.csv")  
  #without rows that didn't have values for speed or NumberClutter

mcelroy_speed_clutter_lme <- lme(Speed ~ NumberClutter, random = ~1|ID, data = fieldspeed_final4)
anova(mcelroy_speed_clutter_lme)  #p-value=0.1097  #not significant
mcelroy_speed_clutter_lme_tukey <- emmeans(mcelroy_speed_clutter_lme, pairwise ~ NumberClutter, adjust = "tukey", data = fieldspeed_final4)
...
anova(lme(Speed ~ NumberClutter + Sex + Substrate.Type, random = ~1|ID, data = fieldspeed_final4))
anova(lme(Speed ~ NumberClutter + Substrate.Type, random = ~1|ID, data = fieldspeed_final4))
anova(lme(Speed ~ NumberClutter + Sex, random = ~1|ID, data = fieldspeed_final4))
#Neither sex nor substrate type affect speed ~ numberclutter
```

```{r}
#Log transformed speed & number clutter
mcelroy_speed_clutter_lme_log <- lme(log10(Speed) ~ log10(NumberClutter+1), random = ~1|ID, data = fieldspeed_final4)
...
```{r}
#Log transformed speed 
mcelroy_speed_clutter_lme_log2 <- lme(log10(Speed) ~ NumberClutter, random = ~1|ID, data = fieldspeed_final4)
...
```{r}
#Log transformed number clutter  -- best model
mcelroy_speed_clutter_lme_log3 <- lme(Speed ~ log10(NumberClutter+1), random = ~1|ID, data = fieldspeed_final4)
...
```{r}
ggplot(fieldspeed_final4, aes(log10(NumberClutter+1), Distance, col=ID)) +
  geom_point()
