...
> does it matter that side_posmax is not normal?
```
...
#histograms
ggplot(data = lab.data, aes(bipedal_side))  + 
...
#bipedal_side: not normal
#bipedal_top: normal

> how does bipedal_side not appear normal here but does below and vice versa for bipedal_top?
```
...
#QQ plot shows data is skewed left
...
```{r}
#Top camera max velocity vs. bipedalism in top
...
```{r}
#Trying to determine if bipedal_side needs to be transformed

#Non-transformed
plotNormalHistogram(as.numeric(lab.data$bipedal_side), main = "Non-transformed")   #looks pretty centered and un-skewed

#Squared
side.dat <- as.numeric(lab.data$bipedal_side)
side.square <- side.dat^2
plotNormalHistogram(side.square, main = "Bipedal_top Squared")   #skewed left

#Exponential
side.exp <- exp(side.dat)
plotNormalHistogram(side.exp, main = "Exponential")   #even more skewed left than squared

#Reciprocal 
side.rec <- 1/side.dat
plotNormalHistogram(side.rec, main = "Reciprocal")  #skewed right
...
```{r}
anova_side_amax <- lm(side_amax ~ sex * substrate_type * bipedal_side, data = lab.data)
anova(anova_side_amax)
```
