...
```{r}
anova_top_vmax <- lm(top_vmax ~ sex * substrate_type * bipedal_top, data = lab.data)
anova(anova_top_vmax)
```

```{r}
aov_top_vmax <- aov(top_vmax ~ sex * substrate_type * bipedal_top, data = lab.data)
posthoc_anova_top_vmax <- TukeyHSD(x = aov_top_vmax, 'sex', conf.level = 0.95, data = lab.data)
```
